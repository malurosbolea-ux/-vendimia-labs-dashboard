"""
Vendimia Labs - Dashboard de prediccion vinicola
Practica final - Aprendizaje automatico y aprendizaje profundo
Equipo: Maria Luisa Ros Bolea - Camilo Gonzalez Chavez - Alvaro Verdasco Robles - Jorge Aldavero Romero
"""
...
st.set_page_config(
    page_title="Vendimia Labs - Prediccion vinicola",
    page_icon="🍷",
    layout="wide",
...
<style>
    ...
    section[data-testid="stSidebar"] .stMarkdown p, section[data-testid="stSidebar"] label {{
        font-family: 'Outfit', sans-serif; color: {CREAM};
    }}
    ...
    .kpi-card {{
        ...
        background: linear-gradient(135deg, rgba(123,13,30,0.35), rgba(26,5,8,0.8));
    ...
    .kpi-card::before {{
        ...
        background: linear-gradient(90deg, transparent, {GOLD}, transparent); opacity: 0.6;
    }}
...
# ── FILTROS ──
df_f = df.copy()
if variedades_sel: df_f = df_f[df_f['VARIEDAD'].isin(variedades_sel)]
if modo_sel: df_f = df_f[df_f['MODO'].isin(modo_sel)]
if color_sel: df_f = df_f[df_f['COLOR'].isin(color_sel)]
if tipo_sel: df_f = df_f[df_f['TIPO'].isin(tipo_sel)]
...
var_pct = ((with_prev['PRODUCCION'].sum() - with_prev['prod_anterior'].sum()) / with_prev['prod_anterior'].sum() * 100) if len(with_prev) > 0 else 0

st.markdown(f"""
<div class="kpi-container">
    <div class="kpi-card"><div class="kpi-value">{fmt(n_parcelas)}</div><div class="kpi-label">Parcelas</div></div>
    <div class="kpi-card"><div class="kpi-value">{fmt(total_prod)} kg</div><div class="kpi-label">Produccion total</div><div class="kpi-delta {'up' if var_pct > 0 else 'down'}">{'▲' if var_pct > 0 else '▼'} {abs(var_pct):.1f}% vs 2020</div></div>
...
with tab2:
    pv = df_f.groupby('VARIEDAD_NOMBRE').agg(mediana=('PRODUCCION','median'), media=('PRODUCCION','mean'), total=('PRODUCCION','sum'), n=('PRODUCCION','count'), sup=('SUPERFICIE','mean')).sort_values('mediana', ascending=True).reset_index()
    fig = go.Figure()
    fig.add_trace(go.Bar(y=pv['VARIEDAD_NOMBRE'], x=pv['mediana'], orientation='h', marker=dict(color=pv['mediana'], colorscale=[[0,BURDEOS3],[0.5,BURDEOS2],[1,GOLD]], line=dict(color=GOLD, width=0.5)), text=pv['mediana'].apply(lambda x: f"{x:,.0f} kg"), textposition='outside', textfont=dict(color=CREAM, size=11)))
    fig = layout_vendimia(fig, "Produccion mediana por variedad", max(450, len(pv)*28))
    fig.update_layout(xaxis_title="Mediana (kg)")
    st.plotly_chart(fig, use_container_width=True)
    st.markdown(f'<div class="section-title">Tabla <span>resumen</span></div>', unsafe_allow_html=True)
    t = pv.rename(columns={'VARIEDAD_NOMBRE':'Variedad','n':'Parcelas','mediana':'Mediana','media':'Media','total':'Total','sup':'Sup. media (ha)'}).sort_values('Total', ascending=False)
    for c in ['Mediana','Media','Total']: t[c] = t[c].apply(lambda x: f"{x:,.0f}")
...
with tab3:
    ...
    if len(wp) > 0:
        ...
        with c1:
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=wp['prod_anterior'], y=wp['PRODUCCION'], mode='markers', marker=dict(color=GOLD, size=5, opacity=0.4, line=dict(width=0.5, color=BURDEOS)), name='Parcelas', hovertemplate="<b>Finca %{{customdata}}</b><br>2020: %{{x:,.0f}} kg<br>2021: %{{y:,.0f}} kg<extra></extra>", customdata=wp['ID_FINCA']))
        ...
        st.info("No hay datos de campana anterior para el filtro actual.")

with tab4:
    st.markdown(f'<div class="section-title">Evolucion del <span>RMSE</span></div>', unsafe_allow_html=True)
    me = pd.DataFrame({'Fase':['Decision Tree','Random Forest','Extra Trees','XGBoost\n(RandomSearch)','LightGBM\n(RandomSearch)','Stacking\n(Ridge)','Voting Ensemble\nv1'],'RMSE':[7595,5774,5663,5594,5649,5636,5516],'Etapa':['Base','Base','Base','Optimizacion','Optimizacion','Ensemble','Ensemble']})
    cm = {'Base':BURDEOS3,'Optimizacion':BURDEOS2,'Ensemble':GOLD}
    fig = go.Figure()
    for e in ['Base','Optimizacion','Ensemble']:
        m = me[me['Etapa']==e]
        fig.add_trace(go.Bar(x=m['Fase'], y=m['RMSE'], name=e, marker=dict(color=cm[e], line=dict(color=GOLD if e=='Ensemble' else "rgba(201,168,76,0.3)", width=1.5 if e=='Ensemble' else 0.5)), text=m['RMSE'].apply(lambda x: f"{x:,}"), textposition='outside', textfont=dict(color=GOLD if e=='Ensemble' else CREAM, size=12)))
    ...
    sd = pd.DataFrame({'Variable':['prod_anterior','prod_hist_media','prod_hist_mediana','SUPERFICIE_IMPUTADA','productividad_hist','prod_zona_media','prod_var_mediana','n_campanas','ALTITUD','temp_verano_media','precipit_primavera','humedad_media'],'Importancia':[0.42,0.18,0.10,0.08,0.05,0.04,0.03,0.025,0.02,0.018,0.015,0.012]}).sort_values('Importancia', ascending=True)
    fig = go.Figure()
    fig.add_trace(go.Bar(y=sd['Variable'], x=sd['Importancia'], orientation='h', marker=dict(color=sd['Importancia'], colorscale=[[0,BURDEOS3],[0.4,BURDEOS2],[1,GOLD]]), text=sd['Importancia'].apply(lambda x: f"{x:.1%}"), textposition='outside', textfont=dict(color=CREAM, size=11)))
